    ...
    for transaction in dataSet:
        for item in transaction:
            if not [item] in C1:
                C1.append([item])
    C1.sort()
    # 构建C1候选集,使用的格式frozenset类型。不能修改它们，列出所有的候选项：
...
# 置信度：A→B，计算条件概率P(B|A)，在A发生的同时B发生的概率 p(AB)/P(A)

# 关联规则挖掘是指找出支持度大于等于最小支持度（设定阈值）并且置信度大于等于最小置信度（设定阈值）的所有规则。分为两步：

# 频繁项集产生：其目标是发现满足最小支持度阈值的所有项集，这些项集称作频繁项集。
# 关联规则的产生：其目标是从上一步发现的频繁项集中提取所有高置信度的规则，这些规则称作强规则。（必须满足最小支持度和最小置信度）
